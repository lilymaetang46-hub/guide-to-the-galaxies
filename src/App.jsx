...
function normalizeCalendarSyncConnection(connection) {
...
function summarizeCalendarSyncLinks(rawLinks) {
  const links = Array.isArray(rawLinks) ? rawLinks : [];

  return links.reduce(
    (summary, link) => {
      const status = link?.sync_status || "pending";
...
function App() {
  ...
  const [googleCalendarConnection, setGoogleCalendarConnection] = useState(
    normalizeCalendarSyncConnection(null)
  );
  const [googleCalendarSyncStats, setGoogleCalendarSyncStats] = useState({
    total: 0,
    pending: 0,
    synced: 0,
    failed: 0,
  ...
  function applyIncomingUrl(incomingUrl) {
    ...
    const googleOauthStatus = parsedUrl.searchParams.get("google_calendar_oauth");
    const googleOauthDetail = parsedUrl.searchParams.get("google_calendar_detail");
    ...
    if (googleOauthStatus) {
      ...
      if (typeof window !== "undefined") {
        window.history.replaceState({}, "", cleanedLocation);
      }
      return;
    }
  ...
  async function loadConnectionsData() {
    ...
    const [
      ...
      { data: calendarConnectionRow, error: calendarConnectionError },
      { data: calendarSyncLinkRows, error: calendarSyncLinkError },
    ] =
      await Promise.all([
        supabase
    ...
    if (calendarConnectionError) {
      console.error("Google Calendar connection load error:", calendarConnectionError);
      setConnectionsMessage("Could not load Google Calendar sync settings.");
    } else {
      setGoogleCalendarConnection(normalizeCalendarSyncConnection(calendarConnectionRow));
    }

    if (calendarSyncLinkError) {
      console.error("Google Calendar sync links load error:", calendarSyncLinkError);
    } else {
      setGoogleCalendarSyncStats(summarizeCalendarSyncLinks(calendarSyncLinkRows || []));
    }

    if (!calendarConnectionRow && !calendarConnectionError) {
      setGoogleCalendarConnection(normalizeCalendarSyncConnection(null));
    }
  ...
  async function saveGoogleCalendarConnection(changes) {
    if (!user) return;

    const currentConnection = normalizeCalendarSyncConnection(googleCalendarConnection);
    const nextConnection = {
      ...currentConnection,
      ...changes,
    };
    const nextStatus = nextConnection.status || "needs_setup";
    const payload = {
      user_id: user.id,
      provider: "google",
      status: nextStatus,
      external_account_email: nextConnection.externalAccountEmail.trim() || null,
      external_calendar_id: nextConnection.externalCalendarId.trim() || null,
      external_calendar_name: nextConnection.externalCalendarName.trim() || null,
      sync_appointments: Boolean(nextConnection.syncAppointments),
      sync_reminders: Boolean(nextConnection.syncReminders),
      sync_dated_todos: Boolean(nextConnection.syncDatedTodos),
      last_error: nextConnection.lastError?.trim() || null,
      connected_at:
        nextStatus === "ready"
          ? nextConnection.connectedAt || new Date().toISOString()
          : nextConnection.connectedAt || null,
      updated_at: new Date().toISOString(),
    };
    ...
    if (error) {
      console.error("Google Calendar connection save error:", error);
      setConnectionsMessage("Could not save Google Calendar sync settings.");
      return;
    }

    setGoogleCalendarConnection(normalizeCalendarSyncConnection(data));
    setConnectionsMessage("Google Calendar sync settings saved.");
  }
  ...
  function shouldSyncAppointmentToGoogle(
    ...
    if (itemType === "reminder") {
      return Boolean(connection.syncReminders);
    }

    return Boolean(connection.syncAppointments);
  }

  async function queueGoogleAppointmentSync({
    appointmentId,
    itemType,
    syncStatus = "pending",
    sourceUpdatedAt = new Date().toISOString(),
    connectionOverride = googleCalendarConnection,
  }) {
    if (
      !user ||
      !appointmentId ||
      !shouldSyncAppointmentToGoogle(itemType, connectionOverride)
    ) {
      return false;
    }
    ...
    const { error } = await supabase
      .from("calendar_sync_event_links")
      .upsert(payload, { onConflict: "user_id,provider,source_type,source_record_id" });
  ...
  async function queueExistingAppointmentsForGoogle(
    ...
    const appointmentsToQueue = appointments.filter((item) =>
      shouldSyncAppointmentToGoogle(item.itemType, connectionOverride)
    );
    ...
    const payload = appointmentsToQueue.map((item) => ({
      user_id: user.id,
      provider: "google",
      source_type: "appointment",
    ...
    if (error) {
      console.error("Existing Google Calendar sync queue error:", error);
      setConnectionsMessage("Google Calendar settings saved, but existing appointments were not queued.");
      return 0;
    }
  ...
  async function updateGoogleCalendarSetting(field, value) {
  ...
  async function startGoogleCalendarOAuth() {
    ...
    try {
      const data = await invokeGoogleCalendarAuth("start", {
        returnUrl:
      ...
      if (!data?.authorizeUrl) {
        throw new Error("Google Calendar auth URL was missing.");
      }
      ...
      setConnectionsMessage(
        error instanceof Error ? error.message : "Could not start Google Calendar OAuth."
      );
      setGoogleCalendarAuthLoading(false);
    }
  }

  async function refreshGoogleCalendarChoices() {
    setGoogleCalendarAuthLoading(true);
    ...
    try {
      ...
      if (!calendars.length) {
        ...
        setConnectionsMessage("Google Calendar list refreshed.");
      }
    } catch (error) {
      setConnectionsMessage(
        error instanceof Error ? error.message : "Could not load Google calendars."
      );
    } finally {
      setGoogleCalendarAuthLoading(false);
    }
  ...
  async function prepareGoogleCalendarSync() {
    ...
    setConnectionsMessage(
      "Google Calendar sync scaffolding is ready. OAuth and live event syncing are the next slice."
    );
  }

  async function markGoogleCalendarReady() {
    const hasCalendarSelection =
      googleCalendarConnection.externalCalendarName.trim() ||
      googleCalendarConnection.externalCalendarId.trim();

    if (!hasCalendarSelection) {
      setConnectionsMessage("Choose a destination Google calendar name or ID first.");
      return;
    }
    ...
    const queuedCount = await queueExistingAppointmentsForGoogle(readyConnection);
    if (queuedCount > 0) {
      setConnectionsMessage(
        `Google Calendar sync is ready. Queued ${queuedCount} existing appointment item${queuedCount === 1 ? "" : "s"} for sync.`
      );
    } else {
  ...
  const addAppointment = async () => {
    ...
    setAppointmentNote("");
    const queuedForGoogle = await queueGoogleAppointmentSync({
      appointmentId: data?.id,
      itemType: appointmentType,
      syncStatus: "pending",
      sourceUpdatedAt: savedAt,
    });
    setAppointmentStatusMessage(
      queuedForGoogle ? "Saved and queued for Google Calendar sync." : "Saved."
    );
  ...
  const updateAppointment = async () => {
    ...
    const queuedForGoogle = await queueGoogleAppointmentSync({
      appointmentId: data?.id || editingAppointmentId,
      itemType: appointmentType,
      syncStatus: "pending",
      sourceUpdatedAt: savedAt,
    });
    ...
    setAppointmentStatusMessage(
      queuedForGoogle ? "Updated and queued for Google Calendar sync." : "Updated."
    );
    await loadAppointments();
  };
  ...
  const removeAppointment = async (appointmentId) => {
    ...
    const queuedForGoogle = await queueGoogleAppointmentSync({
      appointmentId,
      itemType: removedItem?.itemType || "appointment",
      syncStatus: "deleted",
      sourceUpdatedAt: new Date().toISOString(),
    });
    setAppointmentStatusMessage(
      queuedForGoogle ? "Removed and queued for Google Calendar cleanup." : "Removed."
    );
  ...
  if (isCardsPreviewMode) {
    ...
    const previewConnectionsApp = {
      ...
      googleCalendarConnection: {
        ...
        provider: "google",
        status: "ready",
        externalAccountEmail: "lily@gmail.com",
        externalCalendarId: "primary",
        externalCalendarName: "Primary calendar",
        syncAppointments: true,
        syncReminders: true,
        syncDatedTodos: false,
